...
class Mine:
    ...
    def generate_batch_frame(self,batch_size):
        ...
        rotation_theta = torch.from_numpy(np.random.rand(batch_size,2).astype(np.float32)*math.pi*2.0).to(self.rendering_device)
        ...
        while True:
            ...
            tmp_R_matrixs,tmp_T_vecs = self.setup_input.get_rts(rotation_theta.reshape((-1,1)),self.rendering_device)

            tmp_R_matrixs = tmp_R_matrixs.reshape((batch_size,2,3,3))
            tmp_T_vecs = tmp_T_vecs.reshape((batch_size,2,3,1))
            ...
            new_rotation_theta = torch.from_numpy(np.random.rand(invalid_num,2).astype(np.float32)*math.pi*2.0).to(self.rendering_device)
            positions[invalid_idxes] = new_positions
            n_2d[invalid_idxes] = new_n2d
            rotation_theta[invalid_idxes] = new_rotation_theta
