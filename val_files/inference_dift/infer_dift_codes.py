...
if __name__ == "__main__":
    ...
    for which_view in range(args.rotate_num):
        ...
        measurments = np.transpose(measurments,(0,2,1))
        measurments = measurments.reshape(-1,args.measurement_len)
        measurments = measurments * args.scalar

        sampled_rotate_angles = torch.from_numpy(sampled_rotate_angles_np[:,[which_view]]).to(inference_device)
        R_matrix,T_vec = setup_input.get_rts(sampled_rotate_angles,inference_device)
        R_matrix = R_matrix.reshape(1,-1)
        T_vec = T_vec.reshape(1,-1)

        rt_vec = torch.cat((R_matrix, T_vec),dim=1)
        print(rt_vec)
        ...
        while True:
            ...
            sampled_rotate_angles = rt_vec.repeat(cur_batch_size,1).to(inference_device)
            with torch.no_grad():
